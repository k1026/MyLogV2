...
**挙動**：
  - **時間ソート**が適用される場合：生成時間に基づいてソート（有効：新しい順(DESC) / 無効：古い順(ASC)）。
  - **タスクソート**が有効な場合：タスク属性を持つセルをグループ化して表示（先頭に集める or 末尾に集める）。他のソート（時間など）を適用した上で、グループ内での順序を維持する。
  - **手動並べ替え**が有効な場合：他の自動ソート（時間、タスク）を無効にする。
...
**色彩**：
  - 背景：透明。
  - アクティブ時：背景：bg-white/20、ツールアイコン：白、ラベル文字色：白（text-white）。
  - 非アクティブ(無効)時：ツールアイコン：灰色（text-white/50）、ラベル文字色：透明（opacity-0）
**装飾**：なし
...
**ラベル**：
  ...
  - 古い順：`OLD`
**状態**：
  - 有効（DESC/新しい順）：生成時間の新しい順に並べる
  - 無効（ASC/古い順）：生成時間の古い順に並べる
**挙動**：有効 (NEW) ↔ 無効 (OLD) の2状態で切り替え。

### タスクソートアイコン
**名称**：タスクソートアイコン
**種類**：トグルボタン
**用途**：タスクセルの完了状態でグループ化を行う。
**形状**：`vertical_align_top` (先頭) と `vertical_align_bottom` (末尾) を切り替え。
**寸法**：28x28px
**ラベル**：
  - 完了優先：`DONE`
  - 未完了優先：`TODO`
**状態**：
  - 無効：タスクによるグループ化を行わない
  - 完了優先：他のソートを適用した上で、完了タスク、未完了タスクの順で先頭に集め、その後に非タスクセルを配置する（タスクの先頭グループ化）
  - 未完了優先：他のソートを適用した上で、非タスクセルを先頭に集め、その後に未完了タスク、完了タスクの順で末尾に集める（タスクの末尾グループ化）
**挙動**：無効 → DONE (完了優先) → TODO (未完了優先) → 無効 の順でループ。
...
**ラベル**：`MOVE`
**状態**：有効 (ON)、無効 (OFF)
**挙動**：クリックで ON/OFF を切り替え。ON の時は時間・タスクソートを強制解除する。
**備考**：ドラッグ＆ドロップによる実体的な並び替え機能は将来実装（現在はモード切替のみ）。
