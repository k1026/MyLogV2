...
カードはログの基本単位であり、複数のCellを束ねるコンテナとして機能します。
...
- **Time Cell**: Cardの作成時にCardの作成時刻を記録したTime属性のセルを自動追加する
  - Time Cellの`name`には空文字を指定、`value`には`yy.mm.dd HH:MM`形式の現在時刻を指定
...
- **展開と折りたたみ**
  ...
  - カードUIを折りたたんだ時点で`name`が空のTaskセル、`name`と`value`が空のTextセルが存在する場合は削除する
...
### 5.3.2.2 ツールアイコン
- 主にセルUIの並びを制御するための機能アイコン群
- ただし**Timeセルは並べ替え対象外**とし常に先頭に表示する
- **【重要】ツールアイコンの機能では明確な言及がない限りCardセルに記録されているセルIDの並びは変更せず、UI上の見た目の並びだけを変更する**。この仕様は徹底すること。
- **記録**：ツールアイコンの状態はlocalStorageに記録され、ツ減るアイコンの状態やソート条件は他のカードUIの表示にも適用する

#### 5.3.2.2.1 生成時間ソートボタン
- 白色のソートアイコンを閉じるボタンの左側に表示する
...
#### 5.3.2.2.2 タスクアイコン
- 白色のタスクアイコンを生成時間ソートアイコンの左側に表示する
- アイコンをタップすると下記の3つの状態を実現する
  - **無効(デフォルト)**：白色のバツ印付きのチェックボックスアイコン。タスクアイコンを無効化した状態。アイコンにバツ印を表示。タップすると未完了タスク優先に切り替わる
...
#### 5.3.2.2.3 手動並べ替えアイコン
- **配置**：白色の並べ替えアイコン(↑↓)をタスクアイコンの左側に表示する
- **条件**：
  - 生成時間ソートアイコンかタスクアイコンのどちらかが有効になったら手動並べ替えアイコンは無効にする。
  - 手動並べ替えアイコンを有効にしたら生成時間ソートアイコンとタスクアイコンを無効にする。
  - カードUIを閉じた時点で手動並べ替えアイコンの状態を無効に更新する
- **状態**：
  - **無効**：白のバツ印つきの並べ替えアイコンを表示する。セルUIの並び替えを無効化した状態でセルUIの手動での並び替え機能を無効化する。タップすると有効に切り替わり、生成時間ソートアイコンとタスクアイコンを無効化し、localStrageに記録しているセルUIの並び替え状態を無効に更新する。
...
- **セルデータ並べ替え機能**
  ...
  - **条件**：並べ替えアイコンとタスクアイコンが両方とも無効状態の時のみ動作する
  - **挙動**：
...
-**配置**
  - カードUIの右下に表示され、キーボードやフッターなどに隠れないよう移動する

#### 5.3.2.4.1 セル追加ボタンの挙動
-**セルの追加**：セル追加ボタンを押すとそのCardセルに新規のセルを追加する
  ...
  - **セルUIの追加位置**：セルUIの追加位置はその時点のソート条件に依存する
    ...
    - 手動並べ替え機能が無効かつ、生成時間順のソートが無効、タスクアイコンも無効な時は、Cardセルの`value`に記録されている順序の位置に従い追加する
    - 手動並べ替え機能が無効かつ、生成時間順のソートまたはタスクアイコンが有効な時はソート位置に従い追加する


#### 5.3.2.4.2 セル追加ボタンのパイメニュー
-**パイメニューの挙動**
  - セル追加ボタンを10ms以上長押しするとパイメニューが表示さる
  - ドラッグして各メニュー上で離すか、選択せずに追加ボタンを離すとメニューは閉じる
- **パイメニューの項目**
 ...
 **背景色** :カードUIの背景の色は子要素のCellのレア度の平均値で決定する。Card Cell と Time Cellのレア度は平均値の計算に含まない。
