...
**色彩**：
**装飾**：
**配置**：[中央]、scroll-padding-top: 64px, scroll-padding-bottom: 80px (ヘッダー・フッター回避)
**状態**：
**挙動**：Y軸スクロール (smooth)、overflow-y-auto。ヘッダー・フッターの表示状態に応じてscroll-paddingを動的に切り替える。
...
**色彩**：
**装飾**：
**配置**：[リストコンテナ]内、padding: 16px (px-4), gap: 12px (mb-3)
**状態**：垂直リスト表示（1カラム）
**挙動**：
...
**種類**：フローティングアクションボタン
**用途**：新規カードの作成
**形状**：角丸正方形 (rounded-xl)、1pxの紫色の枠線 (border-purple-600)
**寸法**：56px x 56px (w-14 h-14)
**文字**：
**色彩**：背景色：白 (bg-white)、アイコン色：紫 (text-purple-600)
**装飾**：紫色のシャドウ (shadow-[0_4px_12px_rgba(168,85,247,0.3)] / shadow-purple-500/30)
**配置**：
  - [右下]
  - 右端から32px (right-8)
  - 下端からの距離：
...
- **ロジック**：
  ...
  - 展開されたカードがヘッダーと重なるのを防ぐため、`scroll-padding` を使用してスクロール位置を調整する
  - ヘッダー/フッターは `z-[60]` かつ `pointer-events-none`（背景のみ）に設定し、展開されたカード (`z-50`) の上部に常に位置しつつ、背後のスクロールを妨げないようにする
