...
**色彩**：
**装飾**：
**配置**：[リストコンテナ]内、display: grid, padding: 16px (p-4), gap: 16px (gap-4)
**状態**：リストモード時 grid-cols-1、タイルモード時 grid-cols-2
**挙動**：
...
- **カード折りたたみ (Collapse)**：
  - `expandedCardId`をクリア
  - **【スクロール】**：折りたたみ時も、対象カードの上端が表示領域の上端に来るようにスクロール位置を調整（カードが画面外に見切れるのを防ぐため）
...
- **設定**：
  - **リストモード**：通常の縦スクロール
  - **グリッドモード**：2カラム表示。各行の高さは行内の最大高さのカードに合わせる
...
- **ロジック**：
  ...
  - 展開されたカードがヘッダーと重なる場合は、Z-Indexまたはスクロール位置調整で対応する
