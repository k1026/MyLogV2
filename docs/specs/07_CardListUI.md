...
## 7.1 基本構成
- **コンテナ**: 画面全体を使用し、ヘッダーとフッターの背後に配置される。ヘッダーとフッターに隠れないよう、上下にパディング（Header: 64px, Footer: 80px）を持たせる。
...
- **カードUIのクリック (Tap)時の動作**：
  - **位置調整** 
    **展開時**：カードUIを展開した場合はそのカードUIの上部が画面上部に揃うように表示位置を調整してからリスト上でカードUIを展開する。履歴操作（pushState）は行わず、リスト内での状態遷移として扱う。
    **折りたたみ時**：カードUIを折りたたむときは画面やリストが動かないように調整する
    　- 画面内のカードUIを折りたたむ場合はその位置で折りたたむ
      - 画面外のカードUIを折りたたむ場合は現在のリスト表示が動かないように調整する
  - **スクロール制限**
    **展開時**：カードUIを展開している場合はカードリストUIのスクロール範囲をそのカードUIの描画範囲に余裕をもたせた範囲に制限する
    **折りたたみ時**：カードUIを折りたたむときはカードリストUIのスクロール範囲制限を解除する
...
**挙動**：
  - 画面スクロールに併せてフローティングし常に画面上に表示される。
  - **フッターとの干渉回避**：フッターが表示されているときは、フッターの上（bottom: 104px程度）に移動し、フッターが非表示のときは画面下端（bottom: 32px程度）に移動するようにアニメーションする。
