...
- 実装は`.agent/skills/guideline/SKILL.md`に従うこと。
- 実装後の成果物の報告は日本語で行うこと。
...
- `.agent/skills/SAT-DD/tdd-first/SKILL.md`を実行し仕様書を元に、実装に必要なテストと空のシグネチャを作成する。
- テストコマンド`npm run test:ai` を実行し、対象のテストが全て論理的にエラー（Red）になるまで`.agent/skills/SAT-DD/tdd-first/SKILL.md`によるテスト実装を繰り返す。
- 全てのテストが論理的にエラー(Red)になったら**Step2**に進む。

### Step 2: Implementation (tdd-fix)
- `.agent/skills/SAT-DD/tdd-fix/SKILL.md`を実行し、全てのテストをパス（Green）させるための最短の実装を行う。
- `npm run test:ai` を実行し全てのテストをパスするまで`.agent/skills/SAT-DD/tdd-fix/SKILL.md`を繰り返す。

### Step 3: Optimization & Hardening (tdd-strict, tdd-refine)
- `.agent/skills/SAT-DD/tdd-strict/SKILL.md`を実行し、**Step2**で変更されたコードを検証し、コードのを最適化する。
- コードの最適化後、`.agent/skills/SAT-DD/tdd-refine/SKILL.md`を実行してテストケースを厳格化する。
