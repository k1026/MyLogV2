...
## Step 3: テスト実行
2.  `npm run test:error-log`コマンドでテストを実行し、`test-error.log`の内容を確認し、**作成した全てのテストが「論理的に失敗（Red）」している**ことを確認する
...
5.  `npm run test:error-log`コマンドでテストを実行し、出力された`test-error.log`の内容を確認し全てのテストをパスしたら**Step 5**に進む。テストでエラーが出た場合は 4. の実装に戻る。テスト自体に問題がある場合は**Step 2**のテストの作成に戻って良い。
...
8. `npm run test:error-log`コマンドでテストを実行し、出力された`test-error.log`の内容を確認し、全てのテストをパスしたら**Step 6**に進む。テストでエラーが出た場合は 6. に戻る。

## Step 6: 最終確認
9. ユーザーに対し、実装内容と最終的なテスト結果を報告する。実装中に見つかった問題も併せて報告する。
